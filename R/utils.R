...
#' Format slide notes with metadata and font styling
#'
#' Calls \code{format_slide_notes()} for the plain-text content, then wraps
#' each line in an \code{officer::fpar} with the requested font properties.
#' Returns an \code{officer::block_list} suitable for \code{ph_with()} and
#' \code{set_notes()}.
#'
#' @param metadata A list containing the metadata (from load_image_metadata)
#' @param font_settings A list with font_name, font_size, bold, italic, underline, font_color
#'
#' @return An \code{officer::block_list}
#' @keywords internal
#' @noRd
format_slide_notes_styled <- function(metadata, font_settings = list()) {
  plain_text <- format_slide_notes(metadata)

  fn  <- font_settings$font_name      %||% "Calibri"
  fs  <- font_settings$font_size      %||% 8
  b   <- font_settings$bold           %||% FALSE
  it  <- font_settings$italic         %||% FALSE
  ul  <- font_settings$underline      %||% FALSE
  fc  <- font_settings$font_color     %||% "#000000"

  lines <- strsplit(plain_text, "\n", fixed = TRUE)[[1]]
  ...
  pars <- lapply(lines, function(line) {
    officer::fpar(officer::ftext(line, prop = fp))
  })
