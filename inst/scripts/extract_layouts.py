...
def generate_layout_images(base_pptx, output_dir):
    ...
    for i, layout in enumerate(prs.slide_layouts):
        ...
        shape_7_count = sum(
            1 for shape in layout.shapes if shape.is_placeholder and shape.placeholder_format.type == 7
        )
        
        logger.debug(f"Layout '{layout_name}' (Index {i}) has {shape_7_count} shape(s) with placeholder type 7")

        if shape_7_count != 1:
            logger.debug(f"Skipping layout '{layout_name}' (Index {i}) since it does not have exactly one shape with placeholder type 7")
            continue

        logger.info(f"Processing layout '{layout_name}' (Index {i}) (exactly 1 shape with placeholder type 7)")
        ...
        for shape in layout.shapes:
            if shape.is_placeholder:
                ...
                if shape.placeholder_format.type == 7:
                    fill_color = (255, 200, 200)  ## Soft red for placeholder
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Extract and visualize PowerPoint slide layouts if they have exactly one object placeholder (type 7)")
